...
這次做個人履歷網站的過程，真的讓我學到蠻多東西的。

* **Git 協作流程**：以前寫 code 可能就是一直改檔案，壞了就不知道怎麼辦，但這次照著流程走，學會開分支 (`branch`) 去做新功能，再用 PR 合併回來，整個開發過程感覺安心很多，也比較不怕改壞東西。
* **版本控制實踐**：特別是在練習版本控制的時候，我在這次作業中使用 `git revert` 回復某個 commit。原因是我原本想把標題改成一個很亮眼的配色，結果推上去才發現跟背景完全不搭，為了安全起見，我就用 revert 指令把它復原，而不是硬去刪除紀錄。
* **AI 輔助開發**：跟 Copilot 一起寫程式也很有趣，像是 CSS 切版這類比較瑣碎的事情，它幫我省了不少時間，讓我更有心力去想怎麼把畫面弄得好看一點。

總體來說，這次不只練了網頁技術，更懂了怎麼好好管理程式碼。
